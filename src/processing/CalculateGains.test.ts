import { calculateGains, getCurrenyHolding } from './CalculateGains';
import { mockHoldings, mockTradesWithUSDRate } from '../mock';
import {ITrade, IHoldings, ICurrencyHolding, METHOD} from '../types'

describe('getCurrencyHolding LIFO', () => {
    test('One Holdings', () => {
        const holdings = mockHoldings(1, 1);
        const currency = Object.keys(holdings)[0];
        const result = getCurrenyHolding(holdings, currency, 1, METHOD.LIFO);
    ...
    test('More then one Holdings', () => {
        const holdings = mockHoldings(1, 5);
        const currency = Object.keys(holdings)[0];
        const result = getCurrenyHolding(holdings, currency, 1, METHOD.LIFO);
...
describe('getCurrencyHolding FIFO', () => {
    ...
    test('default to FIFO', () => {
        const holdings = mockHoldings(1, 3);
        const currency = Object.keys(holdings)[0];
        const result = getCurrenyHolding(holdings, currency, 1);
    ...
    test('One Holdings', () => {
        const holdings = mockHoldings(1, 1);
        const currency = Object.keys(holdings)[0];
        const result = getCurrenyHolding(holdings, currency, 1, METHOD.FIFO);
...
describe('getCurrencyHolding Overflow', () => {
    ...
    test('with one holding', () => {
        const holdings = mockHoldings(1, 1);
        const currency = Object.keys(holdings)[0];
        const result = getCurrenyHolding(holdings, currency, holdings[currency][0].amount + 1);
        expect('deductedHoldings' in result).toBeTruthy();
        expect('newHoldings' in result).toBeTruthy();
        let totalReduced = 0;
        for(const deductedHolding of result.deductedHoldings) {
    ...
    test('with multiple holdings', () => {
        ...
        const result = getCurrenyHolding(holdings, currency, totalAmount);
        expect('deductedHoldings' in result).toBeTruthy();
        expect('newHoldings' in result).toBeTruthy();
        let totalReduced = 0;
        for(const deductedHolding of result.deductedHoldings) {
...
describe('calculateGains', () => {
    ...
    test('1 holding, 1 trade, short term, overflow', () => {
        ...
        expect(result.longTermGain).toBe(0);
        expect(result.shortTermGain).toBe((trades[0].rate - holdings[currency][0].cost) * holdings[currency][0].amount + trades[0].rate * (trades[0].amountSold - holdings[currency][0].amount));
    });
    ...
    test('1 holding, multiple trades, short term, overflow', () => {
        ...
        let estimatedGain = 0;
        for(const trade of trades) {
            estimatedGain += (trade.rate - holdings[currency][0].cost) * holdings[currency][0].amount + trade.rate * (trade.amountSold - holdings[currency][0].amount)
        }
